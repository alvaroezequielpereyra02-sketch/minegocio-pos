...
import { ArrowLeft, Printer, MessageCircle, X, MapPin, Edit, DollarSign, Download, Share2, Loader2, StickyNote, Landmark } from 'lucide-react';
...
export default function TransactionDetail({
    ...
    // --- DISEÑO DE LA BOLETA (ESTILO FACTURA A4 CON DATOS BANCARIOS) ---
    const getTicketElement = () => {
        const styles = {
            ...
            bankSection: "margin-top: 30px; padding: 15px; border: 1px dashed #cbd5e1; border-radius: 6px; font-size: 12px; color: #475569;",
            noteSection: "margin-top: 10px; padding: 15px; background: #fffbeb; border: 1px solid #fcd34d; border-radius: 6px; font-size: 12px; color: #92400e;",
            footer: "margin-top: 60px; text-align: center; font-size: 11px; color: #94a3b8; border-top: 1px solid #f1f5f9; padding-top: 20px;"
        };

        const bankDetails = (transaction.paymentMethod === 'transfer' && storeProfile.cbu) ? `
            <div style="${styles.bankSection}">
                <strong>DATOS PARA TRANSFERENCIA:</strong><br/>
                ${storeProfile.bankName ? `Banco: ${storeProfile.bankName}<br/>` : ''}
                CBU/CVU: ${storeProfile.cbu}<br/>
                Alias: ${storeProfile.alias}
            </div>` : '';
                    ...
                    <div className="p-4 bg-white min-h-[300px]">
                        ...
                        {activeTab === 'details' && (
                            <div className="space-y-4">
                                ...
                                {/* VISUALIZACIÓN DE DATOS BANCARIOS SI ES TRANSFERENCIA */}
                                {transaction.paymentMethod === 'transfer' && storeProfile.cbu && (
                                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                                        <p className="text-xs font-bold text-slate-400 uppercase flex items-center gap-1"><Landmark size={12} /> Datos Transferencia:</p>
                                        <p className="text-xs font-bold text-slate-700">CBU: {storeProfile.cbu}</p>
                                        <p className="text-xs text-slate-500">Alias: {storeProfile.alias}</p>
                                    </div>
                                )}
