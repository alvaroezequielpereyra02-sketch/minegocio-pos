...
export default function Dashboard({ balance, expenses, setIsExpenseModalOpen, handleDeleteExpense, dateRange, setDateRange }) {
    ...
    // Renderizado custom para el Tooltip del gráfico para que se vea limpio
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden pb-24 lg:pb-0 bg-slate-50 -m-4 p-4">
            ...
            <div className="flex-1 overflow-y-auto space-y-6 pr-1 pb-4 custom-scrollbar">
                ...
                <div className="bg-slate-900 p-5 rounded-2xl shadow-xl text-white relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-center relative z-10">
                        <div className="bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
                            <div className="text-[10px] text-slate-400 uppercase mb-1">Efectivo</div>
                            <div className="text-sm font-bold text-emerald-400">${balance.todayCash.toLocaleString()}</div>
                        </div>
                        <div className="bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
                            <div className="text-[10px] text-slate-400 uppercase mb-1">Digital</div>
                            <div className="text-sm font-bold text-blue-400">${balance.todayDigital.toLocaleString()}</div>
                        </div>
                    </div>
                </div>

                {/* 2. ZONA DE GRÁFICOS (FIX PARA MÓVIL APLICADO) */}
                <div className="flex flex-col lg:flex-row gap-6">

                    {/* Gráfico 1: Barras - EVOLUCIÓN */}
                    {/* IMPORTANTE: min-h-[350px] asegura que tenga altura en móvil */}
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[350px] flex-1">
                        ...
                        <div className="flex-1 w-full min-h-0">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={balance.chartData}>
                                    ...
                                    <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v >= 1000 ? (v / 1000).toFixed(0) + 'k' : v}`} width={35} />
                    ...
                    {/* IMPORTANTE: min-h-[350px] asegura que tenga altura en móvil */}
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[350px] lg:w-1/3">
                ...
                {/* 3. KPIS DEL PERIODO (Cuadritos resumen) */}
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white p-3 rounded-xl shadow-sm border border-l-4 border-l-green-500">
                        <div className="text-slate-400 text-[10px] font-bold uppercase mb-1">Ventas</div>
                    ...
                    <div className="bg-white p-3 rounded-xl shadow-sm border border-l-4 border-l-red-500">
                        <div className="text-slate-400 text-[10px] font-bold uppercase mb-1">Gastos</div>
                    ...
                    <div className="bg-white p-3 rounded-xl shadow-sm border border-l-4 border-l-blue-500">
                        <div className="text-slate-400 text-[10px] font-bold uppercase mb-1">Costo</div>
                    ...
                    <div className="bg-white p-3 rounded-xl shadow-sm border border-l-4 border-l-emerald-500">
                        <div className="text-slate-400 text-[10px] font-bold uppercase mb-1">Neto</div>
