        ...
        <div className="fixed inset-0 z-[20000] bg-slate-100/90 backdrop-blur-sm flex justify-center items-center animate-in fade-in duration-200">
            <div className="w-full max-w-2xl h-full sm:h-auto sm:max-h-[85vh] bg-white sm:rounded-2xl shadow-2xl flex flex-col relative">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                    {localItems.map((item, index) => (
                        <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <button onClick={() => updateQty(index, -1)} className="w-10 h-10 rounded-full border border-slate-300 flex items-center justify-center text-slate-600 hover:bg-slate-100 active:bg-slate-200"><Minus size={18} /></button>
                                    <div className="w-8 text-center font-bold text-xl">{item.qty}</div>
                                    <button onClick={() => updateQty(index, 1)} className="w-10 h-10 rounded-full border border-slate-300 flex items-center justify-center text-slate-600 hover:bg-slate-100 active:bg-slate-200"><Plus size={18} /></button>
                                </div>
                            </div>
                    ...
                    {localItems.length === 0 && <div className="text-center py-10 text-slate-400">No hay items en el pedido.</div>}
                </div>
                <div className="p-4 bg-white border-t sm:rounded-b-2xl sticky bottom-0 z-10">
                    <div className="flex justify-between items-center text-lg">
                        ...
                        <span className="font-extrabold text-2xl text-slate-900">${localItems.reduce((acc, i) => acc + (i.price * i.qty), 0).toLocaleString()}</span>
                    </div>
                </div>
            </div>
        </div>
