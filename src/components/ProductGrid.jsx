...
const ProductGrid = memo(function ProductGrid({
    products, addToCart, searchTerm, setSearchTerm, selectedCategory, setSelectedCategory, categories, userData, barcodeInput, setBarcodeInput, handleBarcodeSubmit
}) {
    ...
    const activeSubCategories = useMemo(() => {
        ...
        const parentId = selectedCatObj.parentId || selectedCategory;
    ...
    const Cell = ({ columnIndex, rowIndex, style, data }) => {
        ...
        return (
            <div style={itemStyle}>
                <button onClick={() => addToCart(product)} className="flex flex-col items-start w-full h-full p-0 rounded-xl border bg-white shadow-sm overflow-hidden active:scale-95 transition-transform relative group hover:border-blue-300">
                    <div className="w-full h-32 bg-slate-100 relative shrink-0">
                        {product.imageUrl ? <img src={product.imageUrl} className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.target.src = 'https://via.placeholder.com/150' }} alt={product.name} /> : <div className="w-full h-full flex items-center justify-center text-slate-300"><ImageIcon className="w-8 h-8" /></div>}
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 mb-1 scrollbar-hide shrink-0 px-1">
                <button onClick={() => setSelectedCategory('all')} className={`px-4 py-1.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === 'all' ? 'bg-slate-800 text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600'}`}>Todos</button>
                {categories.filter(c => !c.parentId).map(cat => (
                    <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-4 py-1.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === cat.id || categories.find(c => c.id === selectedCategory)?.parentId === cat.id ? 'bg-blue-600 text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600'}`}>{cat.name}</button>
                ))}
            </div>
            ...
            {activeSubCategories.length > 0 && (
                <div className="flex gap-2 overflow-x-auto pb-2 mb-2 scrollbar-hide shrink-0 px-1 animate-in slide-in-from-top-2 fade-in">
                    <button onClick={() => { const current = categories.find(c => c.id === selectedCategory); if (current?.parentId) setSelectedCategory(current.parentId); }} className={`px-3 py-1 rounded-lg text-xs font-bold whitespace-nowrap border transition-colors ${!categories.find(c => c.id === selectedCategory)?.parentId ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>Todo</button>
                    {activeSubCategories.map(sub => (
                        <button key={sub.id} onClick={() => setSelectedCategory(sub.id)} className={`px-3 py-1 rounded-lg text-xs font-bold whitespace-nowrap border transition-colors ${selectedCategory === sub.id ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>{sub.name}</button>
                    ))}
                </div>
            )}

            {/* CAMBIO: 'pb-16' (64px) para ajustar EXACTO a la altura de la barra de navegación */}
            <div className="flex-1 min-h-0 pb-16 lg:pb-0">
                {filteredProducts.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400"><Search size={48} className="mb-2 opacity-20" /><p>No se encontraron productos</p></div>
                ) : (
                    <AutoSizer>
                        {({ height, width }) => {
