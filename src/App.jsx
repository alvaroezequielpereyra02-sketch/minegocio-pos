...
// IMPORTAMOS EL NUEVO TRANSFERMODAL
import { ExpenseModal, ProductModal, CategoryModal, CustomerModal, StoreModal, AddStockModal, TransactionModal, LogoutConfirmModal, InvitationModal, ProcessingModal, ConfirmModal, TransferModal } from './components/Modals';
...
export default function App() {
  ...
  // ESTADO PARA MODAL TRANSFERENCIA
  ...
  const handleExportData = () => {
    ...
    try {
      ...
      setTimeout(() => {
        requestConfirm("¿Limpiar Base de Datos?", "✅ Reporte descargado.\n\n¿Quieres borrar el historial de ventas y gastos para liberar espacio?\nEsto NO borra productos ni clientes.", async () => { setIsProcessing(true); await purgeTransactions(); setIsProcessing(false); showNotification("🧹 Historial limpiado"); }, true);
      }, 1500);
  ...
  const handleCheckout = async () => {
    ...
    const itemsWithCost = cart.map(i => { const p = products.find(prod => prod.id === i.id); return { ...i, cost: p ? (p.cost || 0) : 0 }; });
    const saleData = {
    ...
    try {
      ...
      // LOGICA NUEVA PARA TRANSFERENCIA
      ...
      clearCart(); setSelectedCustomer(null); setShowMobileCart(false); setIsProcessing(false);
  ...
  const requestConfirm = (title, message, action, isDanger = false) => { setConfirmConfig({ title, message, onConfirm: async () => { setConfirmConfig(null); await action(); }, onCancel: () => setConfirmConfig(null), isDanger }); };
  ...
  const handleInventoryBarcodeSubmit = (e) => { e.preventDefault(); if (!inventoryBarcodeInput) return; const p = products.find(p => p.barcode === inventoryBarcodeInput); if (p) { setScannedProduct(p); toggleModal('stock', true); setTimeout(() => quantityInputRef.current?.focus(), 100); } else { requestConfirm("Producto no existe", "¿Crear nuevo?", () => { setEditingProduct({ barcode: inventoryBarcodeInput }); toggleModal('product', true); }); } setInventoryBarcodeInput(''); };
  const handleFileChange = async (e) => { const f = e.target.files[0]; if (f) { setIsProcessing(true); const base64 = await compressImage(f); setPreviewImage(base64); setIsProcessing(false); } };
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
          <div className="text-center mb-6">{storeProfile.logoUrl ? <img src={storeProfile.logoUrl} className="w-16 h-16 mx-auto mb-2 rounded-xl object-cover" /> : <Store className="mx-auto text-blue-600 mb-2" size={48} />}<h1 className="text-2xl font-bold text-slate-800">{storeProfile.name}</h1></div>
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            {isRegistering && (<><input name="name" required className="w-full p-3 border rounded-lg" placeholder="Nombre" /><div className="grid grid-cols-2 gap-2"><input name="phone" required className="w-full p-3 border rounded-lg" placeholder="Teléfono" /><input name="address" required className="w-full p-3 border rounded-lg" placeholder="Dirección" /></div><input name="inviteCode" required className="w-full p-2 border rounded-lg text-center font-bold uppercase" placeholder="CÓDIGO INVITACIÓN" /></>)}
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden relative">
      ...
      {/* --- NUEVO MODAL DE TRANSFERENCIA POST-VENTA --- */}
      {showTransferInfo && (
        <TransferModal
          onClose={() => setShowTransferInfo(false)}
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full">
          ...
          {activeTab === 'customers' && userData.role === 'admin' && (
            <div className="flex flex-col h-full overflow-hidden lg:pb-0">
              <div className="flex justify-between items-center mb-4 flex-shrink-0">
                ...
                <div className="flex gap-2"><button onClick={() => toggleModal('invitation', true)} className="bg-slate-100 text-slate-600 px-3 py-2 rounded-lg text-sm font-medium flex gap-1"><KeyRound size={16} /> Invitación</button><button onClick={() => { setEditingCustomer(null); toggleModal('customer', true); }} className="bg-blue-600 text-white px-3 py-2 rounded-lg text-sm font-medium flex gap-1"><Plus size={16} /> Cliente</button></div>
              </div>
        ...
        {/* STORE MODAL ACTUALIZADO PARA GUARDAR CBU */}
        {modals.store && (
          <StoreModal
            ...
            onSave={async (e) => {
              ...
              try {
                ...
                await updateStoreProfile({ name: newName, logoUrl: newLogo, ...bankData });
