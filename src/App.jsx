...
// --- COMPONENTE AUXILIAR (Fuera de la App principal) ---
...
export default function App() {
  ...
  // COMPONENTE DE CARRITO (Reutilizable)
  const CartComponent = () => (
        ...
        <main className="flex-1 overflow-hidden p-4 relative z-0 flex flex-col">
            ...
            {activeTab === 'pos' && (
                <div className="flex flex-col h-full lg:flex-row gap-4 overflow-hidden relative">
                    ...
                    {/* CARRITO MÓVIL */}
                    {showMobileCart && <div className="lg:hidden absolute inset-0 z-[60] bg-white flex flex-col animate-in slide-in-from-bottom duration-200"><CartComponent /></div>}
                    {cart.length > 0 && !showMobileCart && (
                        <button onClick={() => setShowMobileCart(true)} className="lg:hidden absolute bottom-20 left-4 right-4 bg-blue-600 text-white p-4 rounded-xl shadow-2xl flex justify-between items-center z-[55] animate-in fade-in zoom-in">
              ...
              <div className="flex flex-col h-full overflow-hidden pb-20 lg:pb-0">
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-l-4 border-l-green-500"><div className="text-slate-500 text-xs font-bold uppercase mb-1">Ventas Cobradas</div><div className="text-2xl font-bold text-green-700">${balance.salesPaid.toLocaleString()}</div></div>
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-l-4 border-l-blue-500"><div className="text-slate-500 text-xs font-bold uppercase mb-1">Costo Mercadería</div><div className="text-2xl font-bold text-blue-600">-${balance.costOfGoodsSold.toLocaleString()}</div></div>
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-l-4 border-l-red-500"><div className="text-slate-500 text-xs font-bold uppercase mb-1">Gastos Operativos</div><div className="text-2xl font-bold text-red-600">-${balance.totalExpenses.toLocaleString()}</div></div>
                        <div className="bg-slate-800 p-4 rounded-xl shadow-lg text-white relative overflow-hidden">
                            <div className="relative z-10"><div className="text-slate-400 text-xs font-bold uppercase mb-1">Ganancia Neta Real</div><div className="text-3xl font-bold text-emerald-400">${balance.netProfit.toLocaleString()}</div><div className="text-[10px] text-slate-400 mt-1">Lo que queda en tu bolsillo</div></div>
                            <DollarSign className="absolute right-2 bottom-2 text-slate-700 w-16 h-16 opacity-20"/>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-xl shadow-sm border"><h3 className="font-bold text-slate-700 mb-4">Estado de Cobros</h3><div className="flex items-end gap-4 h-32"><div className="flex-1 flex flex-col justify-end items-center gap-2 h-full"><div className="w-full bg-green-100 rounded-t-lg relative group transition-all hover:bg-green-200" style={{height: `${Math.min(100, (balance.salesPaid / (balance.salesPaid + balance.salesPending + 1)) * 100)}%`}}><div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold text-green-700 opacity-0 group-hover:opacity-100">${balance.salesPaid}</div></div><span className="text-xs text-slate-500 font-bold">Cobrado</span></div><div className="flex-1 flex flex-col justify-end items-center gap-2 h-full"><div className="w-full bg-orange-100 rounded-t-lg relative group transition-all hover:bg-orange-200" style={{height: `${Math.min(100, (balance.salesPending / (balance.salesPaid + balance.salesPending + 1)) * 100)}%`}}><div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold text-orange-700 opacity-0 group-hover:opacity-100">${balance.salesPending}</div></div><span className="text-xs text-slate-500 font-bold">Por Cobrar</span></div></div></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white p-6 rounded-xl shadow-sm border"><h3 className="font-bold text-slate-700 mb-4">Stock por Categoría</h3><div className="space-y-3">{Object.entries(balance.categoryValues).map(([cat, val]) => (<div key={cat} className="flex justify-between items-center text-sm"><span className="text-slate-600">{cat}</span><span className="font-bold text-slate-800">${val.toLocaleString()}</span></div>))}</div></div>
