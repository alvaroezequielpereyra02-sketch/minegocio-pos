...
export default function App() {
  ...
  // --- FUNCIÓN NUEVA: CAMBIO MANUAL DE CANTIDAD ---
  ...
  const CartComponent = () => (
    <div className="bg-white h-full flex flex-col">
        {/* HEADER: Cambio de nombre a "Compra" */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cart.map(item => (
            <div key={item.id} className="flex items-center gap-3 border-b border-slate-100 pb-3 last:border-0">
              ...
              {/* CANTIDAD: Input manual entre botones */}
              <div className="flex items-center bg-slate-50 rounded-lg border border-slate-200 overflow-hidden">
                <button 
                  onClick={() => updateCartQty(item.id, -1)} 
                  className="w-8 h-8 flex items-center justify-center hover:bg-slate-200 active:bg-slate-300 text-slate-600 transition-colors border-r border-slate-200"
                >
                ...
                <button 
                  onClick={() => updateCartQty(item.id, 1)} 
                  className="w-8 h-8 flex items-center justify-center hover:bg-slate-200 active:bg-slate-300 text-slate-600 transition-colors border-l border-slate-200"
                >
        ...
        <div className="p-4 bg-slate-50 border-t space-y-4 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            ...
            {userData.role === 'admin' && (
              <div className="relative">
                {selectedCustomer ? (
                  ...
                  <div>
                    {/* BÚSQUEDA: Arreglo de foco (focus-within) */}
                    <div className="flex items-center gap-2 border border-slate-300 rounded-lg p-2.5 bg-white transition-all duration-200 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 focus-within:shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                  <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full p-2.5 border rounded-lg bg-white text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer hover:border-blue-400 transition-colors">
                      <option value="cash">💵 Efectivo</option>
                      <option value="transfer">🏦 Transferencia</option>
                  </select>
              </div>

              {/* TOTAL: Texto más grande y grueso */}
    ...
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full">
        ...
        <main className="flex-1 overflow-hidden p-4 relative z-0 flex flex-col">
            {activeTab === 'pos' && (<div className="flex flex-col h-full lg:flex-row gap-4 overflow-hidden relative"><div className="flex-1 flex flex-col min-h-0">
                
                {/* --- BARRA DE BÚSQUEDA CORREGIDA (SOLUCIÓN APLICADA) --- */}
                <div className="mb-3 flex gap-2">
                    <div className="flex-1">
                        {/* Contenedor Flex con Borde y Focus-Within */}
                    ...
                    {/* Lector de Código (Solo Admin) - ESTILO UNIFICADO */}
                    {userData.role === 'admin' && (
