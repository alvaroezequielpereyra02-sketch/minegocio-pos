...
export default function App() {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false); // NUEVO: Menú Hamburguesa
  ...
  const handlePrintTicket = (transaction) => { /* ... Lógica de impresión (resumida) ... */ 
    const date = transaction.date?.seconds ? new Date(transaction.date.seconds * 1000).toLocaleString() : 'Reciente';
    const printWindow = window.open('', '', 'width=350,height=600');
    printWindow.document.write(`<html><head><style>body{font-family:monospace;padding:20px;font-size:12px;text-align:center}</style></head><body><h3>Ticket</h3><p>${date}</p><hr/>${transaction.items.map(i=>`<div>${i.qty} x ${i.name} - $${i.price*i.qty}</div>`).join('')}<hr/><h3>Total: $${transaction.total}</h3><script>window.onload=function(){window.print();setTimeout(function(){window.close()},500)}</script></body></html>`);
  ...
  const handleCheckout = async () => { if (!user || cart.length === 0) return; let finalClient = { id: 'anonimo', name: 'Anónimo', role: 'guest' }; if (userData.role === 'admin' && selectedCustomer) finalClient = { id: selectedCustomer.id, name: selectedCustomer.name, role: 'customer' }; else if (userData.role === 'client') finalClient = { id: user.uid, name: userData.name, role: 'client' }; const saleData = { type: 'sale', total: cartTotal, items: cart.map(i => ({ id: i.id, name: i.name, qty: i.qty, price: i.price })), date: serverTimestamp(), clientId: finalClient.id, clientName: finalClient.name, clientRole: finalClient.role, sellerId: user.uid }; try { const docRef = await addDoc(collection(db, 'stores', appId, 'transactions'), saleData); for (const item of cart) { const p = products.find(prod => prod.id === item.id); if (p) await updateDoc(doc(db, 'stores', appId, 'products', item.id), { stock: Math.max(0, p.stock - item.qty) }); } setCart([]); setSelectedCustomer(null); setCustomerSearch(''); setLastTransactionId({ ...saleData, id: docRef.id, date: { seconds: Date.now() / 1000 } }); setShowCheckoutSuccess(true); } catch (error) { alert("Error venta."); } };
  ...
  const handleExportCSV = () => { if (transactions.length === 0) return alert("No hay datos."); const csv = ["Fecha,Cliente,Total,Productos"].concat(transactions.map(t => `${new Date(t.date?.seconds*1000).toLocaleDateString()},${t.clientName},${t.total},"${t.items?.map(i=>`${i.qty} ${i.name}`).join('|')}"`)).join('\n'); const l = document.createElement('a'); l.href = URL.createObjectURL(new Blob([csv], {type:'text/csv'})); l.download = 'ventas.csv'; l.click(); };
  ...
  const stats = useMemo(() => { let s=0, t=0, v=0; transactions.forEach(x=>{if(x.type==='sale'){s+=x.total;t++}}); products.forEach(p=>v+=p.price*p.stock); return {totalSales:s, totalTrans:t, inventoryValue:v}; }, [transactions, products]);
  ...
  if (!user || !userData) { /* Login Form (Resumido para brevedad, funcionalmente igual) */
    return (
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      {/* HEADER MEJORADO CON MENÚ DE RESPALDO */}
      <header className="bg-white shadow-sm border-b px-4 py-3 flex justify-between items-center z-[100] shrink-0 h-16 relative">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block">
            ...
            <div className={`text-xs px-2 py-0.5 rounded-full inline-block ${userData.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>
        ...
        {isMenuOpen && (
            <div className="absolute top-16 right-0 w-64 bg-white shadow-2xl border-l border-b border-slate-200 p-4 flex flex-col gap-2 z-[101]">
                <div className="text-sm font-bold text-slate-500 mb-2 border-b pb-2">Menú Rápido</div>
                <button onClick={()=>{setActiveTab('pos'); setIsMenuOpen(false);}} className="text-left p-3 hover:bg-blue-50 rounded flex gap-2 items-center"><LayoutDashboard size={16}/> Vender</button>
                {userData.role === 'admin' && (
                    <>
                        <button onClick={()=>{setActiveTab('inventory'); setIsMenuOpen(false);}} className="text-left p-3 hover:bg-blue-50 rounded flex gap-2 items-center"><Package size={16}/> Inventario</button>
                        <button onClick={()=>{setActiveTab('customers'); setIsMenuOpen(false);}} className="text-left p-3 hover:bg-blue-50 rounded flex gap-2 items-center"><Users size={16}/> Clientes</button>
                        <button onClick={()=>{setActiveTab('dashboard'); setIsMenuOpen(false);}} className="text-left p-3 hover:bg-blue-50 rounded flex gap-2 items-center"><TrendingUp size={16}/> Balance</button>
                    </>
                )}
                <button onClick={()=>{setActiveTab('transactions'); setIsMenuOpen(false);}} className="text-left p-3 hover:bg-blue-50 rounded flex gap-2 items-center"><History size={16}/> Historial</button>
                <div className="border-t pt-2 mt-2">
                    <button onClick={handleLogout} className="text-left p-3 hover:bg-red-50 text-red-600 rounded flex gap-2 items-center w-full"><LogOut size={16}/> Cerrar Sesión</button>
                </div>
            </div>
        )}
      </header>
      ...
      <main className="flex-1 overflow-hidden p-4 max-w-5xl mx-auto w-full relative z-0">
        ...
        {/* ... (Las otras vistas Inventory, Customers, Dashboard, Transactions se mantienen igual) ... */}
        ...
        {activeTab === 'dashboard' && userData.role === 'admin' && (<div className="h-full overflow-y-auto pb-20 lg:pb-0"><h2 className="text-xl font-bold text-slate-800 mb-6">Balance</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="bg-blue-600 rounded-2xl p-6 text-white shadow-lg"><div className="text-3xl font-bold">${stats.totalSales.toLocaleString()}</div><div className="opacity-80 text-sm">Ventas</div></div><div className="bg-white rounded-2xl p-6 shadow-sm border"><div className="text-3xl font-bold text-slate-800">${stats.inventoryValue.toLocaleString()}</div><div className="text-slate-500 text-sm">Valor Stock</div></div></div></div>)}
               ...
               {transactions.map(t => (
                 <div key={t.id} className="p-3 flex justify-between items-center hover:bg-slate-50 text-sm">
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     <button onClick={() => handlePrintTicket(t)} className="p-2 bg-slate-100 rounded-full hover:bg-blue-100 text-slate-600 transition-colors"><Printer size={16} /></button>
                   </div>
                 </div>
               ))}
             </div>
      ...
      {/* NAVEGACIÓN INFERIOR (CON Z-INDEX ALTO) */}
      ...
      {/* NAVEGACIÓN ESCRITORIO (CON Z-INDEX ALTO) */}
