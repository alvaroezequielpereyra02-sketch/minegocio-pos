...
export default function App() {
  ...
  const [isLogoutModalOpen, setIsLogoutModalOpen] = useState(false); // NUEVO ESTADO DE MODAL
  ...
  const balance = useMemo(() => {
    ...
    return { salesPaid, salesPending, salesPartial, inventoryValue, totalExpenses, grossProfit, netProfit, categoryValues, costOfGoodsSold, todayCash, todayDigital, todayTotal, chartData };
  }, [transactions, products, expenses, categories]);

  const getCustomerDebt = (customerId) => {
    return transactions.filter(t => t.clientId === customerId && t.paymentStatus === 'pending').reduce((acc, t) => acc + t.total, 0);
  };
  ...
  const handleResetPassword = async () => {
    ...
    if (!email) { setLoginError("Por favor, escribe tu correo arriba primero."); return; }
    try { await sendPasswordResetEmail(auth, email); alert("📧 ¡Listo! Revisa tu correo (y Spam) para restablecer la contraseña."); setLoginError(""); } catch (error) { setLoginError("Error: No pudimos enviar el correo."); }
  };

  // --- FUNCIÓN ORIGINAL MODIFICADA PARA ABRIR MODAL ---
  const handleLogout = () => {
    setIsLogoutModalOpen(true); // Abre el modal en lugar de la función confirm()
  };
  ...
  const handleShareWhatsApp = async (transaction) => {
    ...
    try { const pdfBlob = await html2pdf().set(opt).from(element).output('blob'); const file = new File([pdfBlob], `ticket-${transaction.id.slice(0,5)}.pdf`, { type: 'application/pdf' }); if (navigator.canShare && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'Comprobante de Venta', text: `Hola! Aquí tienes tu comprobante de compra en ${storeProfile.name}.`, }); } else { alert("Tu dispositivo no soporta compartir archivos directamente. El archivo se descargará."); html2pdf().set(opt).from(element).save(); } } catch (error) { console.error("Error al compartir:", error); alert("Hubo un error al intentar compartir. Intenta descargando el ticket."); }
  };
  ...
  const handleExportCSV = async () => {
    ...
    setTimeout(async () => {
        if (confirm("⚠️ ATENCIÓN ⚠️\n\n¿Se descargó correctamente el archivo?\n\nSi deseas limpiar el sistema para un nuevo período, acepta aquí para ELIMINAR TODO EL HISTORIAL.\n\n(Esta acción no se puede deshacer)")) {
            ...
            if (password === "BORRAR") { try { const promises = transactions.map(t => deleteDoc(doc(db, 'stores', appId, 'transactions', t.id))); await Promise.all(promises); alert("✅ Historial eliminado correctamente."); } catch (error) { alert("Error al borrar: " + error.message); } } else { alert("Operación cancelada. El historial no se borró."); }
        }
    }, 1000);
  };
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const row of data) {
            ...
            const barcode = (row['Codigo'] || row['codigo'] || row['CODE'] || '').toString();
            if (name && price > 0) {
                await addDoc(collection(db, 'stores', appId, 'products'), {
                    name, price, cost, stock, barcode,
                    categoryId: '', imageUrl: '', createdAt: serverTimestamp()
                });
                count++;
            }
        }
        ...
        console.error(error);
        alert("Error al leer el archivo. Asegúrate que sea un Excel válido (.xlsx o .xls).");
      }
    };
  ...
  const CartComponent = () => (
    <div className="bg-white h-full flex flex-col">
        ...
        <div className="p-4 bg-slate-50 border-t space-y-3">
            ...
            <div className="flex gap-2"><select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full p-2 border rounded bg-slate-50 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500"><option value="cash">💵 Efectivo</option><option value="transfer">🏦 Transferencia</option><option value="card">💳 Tarjeta</option><option value="qr">📱 QR</option></select></div>
...
      
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[110] backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 space-y-4 shadow-2xl text-center border-t-4 border-red-500">
            <LogOut size={32} className="text-red-500 mx-auto"/>
            <h3 className="font-bold text-lg text-slate-800">Confirmar Cierre de Sesión</h3>
            <p className="text-sm text-slate-600">¿Estás seguro de que quieres cerrar tu sesión actual? Deberás ingresar tus credenciales para volver a entrar.</p>
            <div className="flex gap-3 pt-2">
              <button 
                onClick={() => setIsLogoutModalOpen(false)} 
                className="flex-1 py-2 text-slate-700 font-bold bg-slate-100 rounded-lg hover:bg-slate-200"
              >
                Cancelar
              </button>
              <button 
