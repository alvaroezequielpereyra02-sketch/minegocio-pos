...
export default function App() {
  ...
  const [showMobileCart, setShowMobileCart] = useState(false); // NUEVO: Carrito Móvil
  ...
  // COMPONENTE DE CARRITO (Reutilizable para Desktop y Mobile)
  const CartComponent = () => (
    <div className="bg-white h-full flex flex-col">
        <div className="p-4 border-b bg-slate-50 font-bold text-slate-700 flex justify-between items-center">
            <span className="flex gap-2"><ShoppingCart className="w-5 h-5" /> Ticket</span>
            {/* Botón cerrar solo para móvil */}
            <button onClick={() => setShowMobileCart(false)} className="lg:hidden p-1 bg-slate-200 rounded-full"><X size={20}/></button>
        </div>
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      ...
      {/* SIDEBAR PARA PC (SOLUCIÓN DEFINITIVA INTERFAZ PC) */}
      <div className="hidden lg:flex flex-col w-64 bg-white border-r z-20 shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-1">
            <button onClick={() => setActiveTab('pos')} className={`w-full text-left p-3 rounded-lg flex gap-3 items-center font-medium ${activeTab==='pos'?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}><LayoutDashboard size={20}/> Vender</button>
            {userData.role === 'admin' && (
                <>
                    <button onClick={() => setActiveTab('inventory')} className={`w-full text-left p-3 rounded-lg flex gap-3 items-center font-medium ${activeTab==='inventory'?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}><Package size={20}/> Inventario</button>
                    <button onClick={() => setActiveTab('customers')} className={`w-full text-left p-3 rounded-lg flex gap-3 items-center font-medium ${activeTab==='customers'?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}><Users size={20}/> Clientes</button>
                    <button onClick={() => setActiveTab('dashboard')} className={`w-full text-left p-3 rounded-lg flex gap-3 items-center font-medium ${activeTab==='dashboard'?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}><TrendingUp size={20}/> Balance</button>
                </>
            )}
            <button onClick={() => setActiveTab('transactions')} className={`w-full text-left p-3 rounded-lg flex gap-3 items-center font-medium ${activeTab==='transactions'?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}><History size={20}/> Historial</button>
        </nav>
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full">
        <header className="lg:hidden bg-white shadow-sm border-b px-4 py-3 flex justify-between items-center z-[50] shrink-0 h-16">
            ...
            <div className="flex gap-3">
                <button onClick={handleLogout} className="bg-slate-100 p-2 rounded-full text-slate-600"><LogOut size={18}/></button>
            </div>
        </header>
        ...
        <main className="flex-1 overflow-hidden p-4 relative z-0 flex flex-col">
            {activeTab === 'pos' && (
                <div className="flex flex-col h-full lg:flex-row gap-4 overflow-hidden relative">
                    {/* LISTA PRODUCTOS (Ocupa todo en móvil) */}
                    <div className="flex-1 flex flex-col min-h-0">
                        <div className="mb-3 flex gap-2">
                            <div className="relative flex-1"><Search className="absolute left-3 top-3 text-slate-400 w-5 h-5" /><input className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 pb-20 lg:pb-0">
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()) && (selectedCategory === 'all' || p.categoryId === selectedCategory)).map(product => (
                                    <button key={product.id} onClick={() => addToCart(product)} className="flex flex-col items-start p-0 rounded-xl border bg-white shadow-sm overflow-hidden active:scale-95 transition-all relative">
                    ...
                    {/* CARRITO PC (Visible siempre a la derecha) */}
                    <div className="hidden lg:block w-80 rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                        <CartComponent />
                    </div>

                    {/* CARRITO MÓVIL (MODAL OVERLAY) */}
                    {showMobileCart && (
                        <div className="lg:hidden absolute inset-0 z-[60] bg-white flex flex-col animate-in slide-in-from-bottom duration-200">
                            <CartComponent />
                        </div>
                    )}
                    ...
                    {cart.length > 0 && !showMobileCart && (
                        <button 
                            ...
                            className="lg:hidden absolute bottom-20 left-4 right-4 bg-blue-600 text-white p-4 rounded-xl shadow-2xl flex justify-between items-center z-[55] animate-in fade-in zoom-in"
                        >
                            <div className="flex items-center gap-2 font-bold"><ShoppingCart size={20}/> Ver Pedido ({cart.reduce((a,b)=>a+b.qty,0)})</div>
                            <div className="font-bold text-lg">${cartTotal} <ChevronRight size={18} className="inline"/></div>
                        </button>
                    )}
                </div>
            )}

            {/* VISTA BALANCE AVANZADO */}
            {activeTab === 'dashboard' && userData.role === 'admin' && (
              <div className="flex flex-col h-full overflow-hidden pb-20 lg:pb-0">
                <div className="flex justify-between items-center mb-6 flex-shrink-0">
                    ...
                    <button onClick={() => setIsExpenseModalOpen(true)} className="bg-red-100 text-red-600 px-3 py-2 rounded-lg flex items-center gap-2 text-sm font-bold hover:bg-red-200"><Wallet size={16}/> Gasto</button>
                </div>
                <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="bg-slate-800 p-4 rounded-xl shadow-lg text-white relative overflow-hidden">
                            <div className="relative z-10">
                                <div className="text-slate-400 text-xs font-bold uppercase mb-1">Ganancia Neta Real</div>
                                <div className="text-3xl font-bold text-emerald-400">${balance.netProfit.toLocaleString()}</div>
                                <div className="text-[10px] text-slate-400 mt-1">Lo que queda en tu bolsillo</div>
                            </div>
                            <DollarSign className="absolute right-2 bottom-2 text-slate-700 w-16 h-16 opacity-20"/>
                        </div>
                    </div>
                    
                    <div className="bg-white p-6 rounded-xl shadow-sm border">
                        <h3 className="font-bold text-slate-700 mb-4">Estado de Cobros</h3>
                        <div className="flex items-end gap-4 h-32">
                            <div className="flex-1 flex flex-col justify-end items-center gap-2 h-full"><div className="w-full bg-green-100 rounded-t-lg relative group transition-all hover:bg-green-200" style={{height: `${Math.min(100, (balance.salesPaid / (balance.salesPaid + balance.salesPending + 1)) * 100)}%`}}><div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold text-green-700 opacity-0 group-hover:opacity-100">${balance.salesPaid}</div></div><span className="text-xs text-slate-500 font-bold">Cobrado</span></div>
                            <div className="flex-1 flex flex-col justify-end items-center gap-2 h-full"><div className="w-full bg-orange-100 rounded-t-lg relative group transition-all hover:bg-orange-200" style={{height: `${Math.min(100, (balance.salesPending / (balance.salesPaid + balance.salesPending + 1)) * 100)}%`}}><div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold text-orange-700 opacity-0 group-hover:opacity-100">${balance.salesPending}</div></div><span className="text-xs text-slate-500 font-bold">Por Cobrar</span></div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border">
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">{expenses.map(exp => (<div key={exp.id} className="flex justify-between text-sm p-2 hover:bg-slate-50 rounded"><div><div className="font-medium text-slate-700">{exp.description}</div><div className="text-xs text-slate-400">{new Date(exp.date?.seconds * 1000).toLocaleDateString()}</div></div><div className="flex items-center gap-2"><span className="font-bold text-red-500">-${exp.amount}</span><button onClick={() => handleDeleteExpense(exp.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={12}/></button></div></div>))}</div>
                        </div>
                    </div>
                </div>
              </div>
            ...
            {/* OTRAS VISTAS (Inventory, Customers, Transactions - Código idéntico pero dentro del nuevo layout) */}
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex justify-around items-center z-[50] shadow-lg">
            ...
            <NavButton active={activeTab === 'transactions'} onClick={() => {setActiveTab('transactions'); setHistorySection('menu');}} icon={<History size={24} />} label="Historial" />
        ...
        {/* MODALES COMPARTIDOS (Producto, Gasto, etc) - Código igual a versiones anteriores para mantener funciones */}
        {/* Modal Gasto */}
        ...
        {/* Otros modales necesarios para funcionalidad completa */}
