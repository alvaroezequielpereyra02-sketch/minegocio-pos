...
import { 
  ...
  Link as LinkIcon, Download, Tags, LogOut, Users, MapPin, Phone, Printer // Icono Impresora
} from 'lucide-react';
...
export default function App() {
  ...
  const [lastTransactionId, setLastTransactionId] = useState(null); // Para imprimir último ticket
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const newUserData = { email: form.email.value, name: form.name.value, phone: form.phone.value, address: form.address.value, role, createdAt: serverTimestamp() };
      await setDoc(doc(db, 'users', userCredential.user.uid), newUserData);
      if(role === 'client') await addDoc(collection(db, 'stores', appId, 'customers'), { name: form.name.value, phone: form.phone.value, address: form.address.value, email: form.email.value, createdAt: serverTimestamp() });
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="info">
            Fecha: ${date}<br/>
            Cliente: ${transaction.clientName || 'Consumidor Final'}<br/>
            Atendido por: ${transaction.sellerId === user.uid ? 'Mí' : 'Caja'}
          </div>
          <table>
  ...
  const handleCheckout = async () => {
    ...
    const saleData = { type: 'sale', total: cartTotal, items: cart.map(i => ({ id: i.id, name: i.name, qty: i.qty, price: i.price })), date: serverTimestamp(), clientId: finalClient.id, clientName: finalClient.name, clientRole: finalClient.role, sellerId: user.uid };
    ...
    try {
      ...
      setLastTransactionId({ ...saleData, id: docRef.id, date: { seconds: Date.now() / 1000 } }); // Guardar para imprimir
      setShowCheckoutSuccess(true); 
    } catch (error) { alert("Error venta."); }
  };

  // --- GESTIÓN CRUD (Igual que antes) ---
  const handleSaveCustomer = async (e) => {
    e.preventDefault(); const form = e.target; const customerData = { name: form.name.value, phone: form.phone.value, address: form.address.value, email: form.email.value };
    try { if(editingCustomer) await updateDoc(doc(db, 'stores', appId, 'customers', editingCustomer.id), customerData); else await addDoc(collection(db, 'stores', appId, 'customers'), { ...customerData, createdAt: serverTimestamp() }); setIsCustomerModalOpen(false); } catch (e){alert("Error");}
  };
  ...
  const handleExportCSV = () => {
    ...
    const csv = ["Fecha,Cliente,Total,Productos"].concat(transactions.map(t => `${new Date(t.date?.seconds*1000).toLocaleDateString()},${t.clientName},${t.total},"${t.items?.map(i=>`${i.qty} ${i.name}`).join('|')}"`)).join('\n');
  ...
  if (!user || !userData) { /* PANTALLA DE LOGIN IGUAL QUE ANTES */
    return (
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      <header className="bg-white shadow-sm border-b px-4 py-3 flex justify-between items-center z-30 shrink-0 h-16">
        ...
        <div className="flex items-center gap-4"><div className="text-right hidden sm:block"><div className="text-sm font-bold">{userData.name}</div><div className="text-xs text-slate-500 capitalize">{userData.role === 'admin' ? 'Admin' : 'Cliente'}</div></div><button onClick={handleLogout} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><LogOut size={18}/></button></div>
      </header>
      ...
      <main className="flex-1 overflow-hidden p-4 max-w-5xl mx-auto w-full relative">
        ...
        {/* ... Resto de vistas Inventory, Customers, Dashboard (Iguales) ... */}
        ...
        {/* HISTORIAL CON BOTÓN DE IMPRIMIR */}
               ...
               {transactions.map(t => (
                 <div key={t.id} className="p-3 flex justify-between items-center hover:bg-slate-50 text-sm">
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     {/* BOTÓN IMPRIMIR */}
                     <button onClick={() => handlePrintTicket(t)} className="p-2 bg-slate-100 rounded-full hover:bg-blue-100 text-slate-600 hover:text-blue-600 transition-colors" title="Imprimir Ticket">
                        <Printer size={16} />
                     </button>
                   </div>
                 </div>
