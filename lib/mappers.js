...
// =============================================================================
// Fase 3A — Transacciones
// =============================================================================

export function mapTransactionItem(row) {
  if (!row) return null;
  return {
    id:            row.id,
...
// El resto de la app (calcBalance, TransactionDetail, exportes a PDF) trata
// `date` como algo que se le puede pasar a `new Date(...)` directo, o que
// puede tener `.seconds` (formato Timestamp de Firestore) — ya maneja los
// dos casos con un fallback. Un string ISO de Postgres entra justo por el
// segundo camino, así que no hace falta simular la forma de Firestore.
export function mapTransaction(row, items = []) {
  if (!row) return null;
  return {
    id:                row.id,
    type:              row.type,
    ...
    status:            row.fulfillment_status,       // ver nota en migration_03: el campo real que usan Pedidos/Reparto
    ...
    clientInfo: {                                    // compatibilidad con componentes que leen la forma anidada
